...
class Window(QtGui.QMainWindow):
    def __init__(self, parent=getMayaWindow()):
        ...
        filter = ['*.jpeg', '*.jpg', '*.gif', '*.iff', '*.png', '*.psd', '*.tga', '*.tif', '*.bmp', '*.JPEG', '*.JPG',
                  '*.GIF', '*.IFF', '*.PNG', '*.PSD', '*.TGA', '*.TIF', '*.BMP']
        ...
        self.fileBrowserModel.setFilter(QtCore.QDir.AllDirs | QtCore.QDir.NoDotAndDotDot | QtCore.QDir.AllEntries)
        self.fileBrowserModel.setNameFilters(filter)
        self.fileBrowserModel.setNameFilterDisables(0)
        ...
        self.createTextureFilesButton.setMask(
            getRoundedMask(self.createTextureFilesButton.width(), self.createTextureFilesButton.height()))
        ...
        self.layeredTextureTreeView.setColumnWidth(0, 400)
        self.layeredTextureTreeView.setColumnWidth(1, 50)
        ...
        self.openHyperShadeButton.setMask(
            getRoundedMask(self.openHyperShadeButton.width(), self.openHyperShadeButton.height()))
        ...
        self.assignToSelectedButton.setMask(
            getRoundedMask(self.assignToSelectedButton.width(), self.assignToSelectedButton.height()))
        ...
        self.channelComboBox.addItems(
            ["Diffuse", "Weight", "Transparency", "Reflectivity", "Glossiness", "Cutout Opacity", "Bump", "Normal"])
        ...
        assignMaterialCmd = lambda: self.assignToSelected(self.layeredTextureTreeView.selectedIndexes(),
                                                          self.channelComboBox.currentIndex())
    ...
    def _createMenuBar(self):
        ...
        self.settingsMenuItem = self.windowMenuBar.addMenu("Options")
        ...
        self.alphaIsLuminanceCheckBox = self.settingsMenuItem.addAction("Alpha is Luminance")
        self.filterOffCheckBox = self.settingsMenuItem.addAction("Filter Type off")

        self.layerTexturesCheckBox.setCheckable(True)
        self.previewTexturesCheckBox.setCheckable(True)
        self.alphaIsLuminanceCheckBox.setCheckable(True)
        self.filterOffCheckBox.setCheckable(True)
        self.previewTexturesCheckBox.setChecked(True)

        self.alphaIsLuminanceCheckBox.setChecked(True)
        self.filterOffCheckBox.setChecked(True)
    ...
    def fileBrowserIndexChanged(self, indexes):
        ...
        if self.previewTexturesCheckBox.isChecked():
            for index, file in enumerate(list(set(imageList))):
                ...
                texturePreviewLabel = TexturePreview(file, fileName, parent=self.main_widget)
    ...
    def createFileTextureNodes(self):
        ...
        newTextureSet = TextureSet(layeredTextureName, alphaIsLuminance=self.alphaIsLuminanceCheckBox.isChecked(),
                                   filterOff=self.filterOffCheckBox.isChecked())
    ...
    def assignToSelected(self, layerIndex, channelIndex):
        for obj in pm.selected():
            ...
            #
            #
            # options = {"layerTextures" : self.layerTexturesCheckBox.isChecked(),
            # 			   "previewTextures" : self.previewTexturesCheckBox.isChecked(),
            # 			   "filterOff" : self.filterOffCheckBox.isChecked()}
...
class TexturePreview(QtGui.QLabel):
    def __init__(self, imageDir, imageName, parent=None):
        ...
        self.pixmapThread = _pixmapFromFileThread(imageDir, parent=parent)
        self.pixmapThread.start()
        self.connect(self.pixmapThread, QtCore.SIGNAL("_updatePixmap(QImage)"), self._updatePixmap)
        self.setFixedSize(95, 95)

    def _updatePixmap(self, scaledImage):
        pixmap = QtGui.QPixmap()
        pixmap.convertFromImage(scaledImage)
        self.setPixmap(pixmap)
        if self.pixmap().isNull():
            self.setText(self.imageName)


class TextureSet(QtGui.QStandardItem):
    def __init__(self, layeredTextureName, **kwargs):
        ...
        self.alphaIsLuminance = kwargs.pop("alphaIsLuminance", True)
        self.filterOff = kwargs.pop("filterOff", True)
    ...
    def createSet(self, imageList, createLayered, layeredTextureName):
        ...
        if createLayered:
            ...
            self.LayeredTexture = pm.shadingNode("layeredTexture", asTexture=True,
                                                 n=layeredTextureName + "_layeredTexture")

            if self.alphaIsLuminance:
        ...
        # Search for alpha textures
            ...
            for i, file in alphaList:
                ...
                newName = os.path.basename(file)
                newFileTextureNode = Texture(newName, filterOff=self.filterOff)
                newFileTextureNode.fileTextureName = file
                self.nodeList.append(newFileTextureNode.node)
...
class _pixmapFromFileThread(QtCore.QThread):
    def __init__(self, imageDir, parent=None):
        super(_pixmapFromFileThread, self).__init__(parent)
    ...
    def run(self):
        ...
        reader = QtGui.QImageReader(self._imageDir)
        reader.setScaledSize(QtCore.QSize(95, 95))
        scaledImage = QtGui.QImage(reader.read())
        mutex.unlock()

        self.emit(QtCore.SIGNAL("_updatePixmap(QImage)"), scaledImage)
...
def createBump(node, mat, normal=False, mode="overall_bump", **kwargs):
    kwargs.pop("mode", "overall_bump")
    kwargs.pop("")
    if mode != "overall_bump":
        mode = mode
    else:
        mode = "overall_bump"

    node.alphaIsLuminance.set(1)
    newBumpNodeName = node + '_bump'
    newBumpNode = pm.shadingNode("bump2d", asUtility=True, n=newBumpNodeName)

    if normal == True:
        newBumpNode.bumpInterp.set(2)
    else:
        pass

    pm.connectAttr(node + ".outAlpha", newBumpNode + '.bumpValue', f=True)
    pm.connectAttr(newBumpNode + '.outNormal', mat + '.' + mode, f=True)
